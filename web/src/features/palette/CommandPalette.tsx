...
// Owns the query/combobox state. Rendered only while the Modal is open (see
// CommandPalette below) so it mounts fresh every time the palette opens —
// query starts at '' and the combobox starts closed by construction,
// instead of an effect resetting them each time `opened` flips true.
function CommandPaletteBody({ actions }: { actions: PaletteAction[] }) {
  ...
  // Mount-only: this component exists only while the palette is open, so
  // opening the dropdown here is equivalent to opening it whenever the
  // palette opens.
  useEffect(() => {
    ...
    >
      <Combobox.Target withExpandedAttribute={false}>
        <TextInput
          ...
          onKeyDown={(event) => {
            // The combobox's own Escape handling only clears its internal
            // dropdownOpened flag (Options always renders regardless -- see
            // the module comment above), which leaves Mantine's Modal
            // unable to close on Escape itself: useModal's closeOnEscape
            // skips onClose whenever the event target carries
            // data-mantine-stop-propagation (set here by the combobox
            // while its dropdown is open, which by this component's design
            // is the entire time the palette is open) -- a convention
            // meant for Escape to close an inner overlay before an outer
            // one, but with no Combobox.Dropdown to close here it would
            // otherwise take two Escape presses. Close the palette
            // directly instead.
  ...
  return (
    ...
    >
      ...
      <Modal.Content aria-label="Command palette">
        <Modal.Body p={0}>
          <CommandPaletteBody actions={actions} />
        </Modal.Body>
      </Modal.Content>
    </Modal.Root>
  )
