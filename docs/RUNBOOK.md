...
| Start/stop/upgrade fails with `sudo: The "no new privileges" flag is set` or `sudo: unable to change to root gid: Operation not permitted` | The manager unit carries the v1.3.0–v1.16.6 hardening block: its seccomp-backed options (`PrivateDevices`, `ProtectKernel*`, `SystemCallArchitectures`, …) make systemd before v255 (Debian 12, Ubuntu 22.04) set `no_new_privs` on the unit, so the setuid `sudo` refuses to run (first message), and its empty `CapabilityBoundingSet=` strips the root that `sudo` becomes of every capability on any systemd version (second message). Re-run `install.sh`: it rewrites the unit with mount-namespace options only and restarts the manager. Check with `systemctl show valheim-ui -p NoNewPrivileges -p CapabilityBoundingSet`. |
...
progress every 2 s and pushes it on the event stream, and the Map tab
reloads the mask whenever its version changes.
