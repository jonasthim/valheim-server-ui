...
// maxCharacterFileBytes bounds an uploaded .fch (they are a few hundred KB).
const maxCharacterFileBytes = 32 << 20

// ImportExplored forwards a character file to id's agent, which merges the
// map the character explored in this world into the fog.
func (s *Service) ImportExplored(ctx context.Context, id string, file []byte) (*domain.ExploredImportResult, error) {
	if len(file) == 0 || len(file) > maxCharacterFileBytes {
		return nil, domain.Ef(domain.CodeValidationFailed, "character file must be between 1 byte and %d MB", maxCharacterFileBytes>>20)
	}
	in, err := s.inst.Get(ctx, id)
	if err != nil {
		return nil, err
	...
	connected := x != nil && x.connected
	ms := s.maps[id]
	unsupported := ms != nil && (ms.fogUnsupported || ms.unsupported)
	s.mu.Unlock()
	if !connected {
		return nil, domain.E(domain.CodeConflict, "the agent is not connected; the server must be running to import exploration")
	}
	if unsupported {
		return nil, domain.E(domain.CodeConflict, "the running agent has no fog support; update it from the Mods tab")
	}
	c, err := s.client(paths, in.Config.Port)
	if err != nil {
		return nil, err
	}
	res, err := c.ImportExplored(ctx, file)
	if err != nil {
		if isNotFound(err) {
			return nil, domain.E(domain.CodeConflict, "the running agent cannot import character files; update it from the Mods tab")
		}
		return nil, domain.Ef(domain.CodeUpstreamError, "the agent refused the import: %v", err)
	}
	// Forget the cached fog state so the next poll and mask fetch see the change.
	s.mu.Lock()
	if ms != nil {
		ms.explored, ms.exploredAt = nil, time.Time{}
	}
	s.mu.Unlock()
	return res, nil
}
