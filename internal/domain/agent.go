...
// round-trip. The command membership (req.Command against AgentCommands) is
// checked by the caller.
func ValidateAgentCommand(req AgentCommandRequest) []FieldError {
	var fields []FieldError
	switch req.Command {
	case "kick", "ban", "unban":
		if req.Target == "" || len(req.Target) > 64 {
			fields = append(fields, FieldError{Field: "target", Message: "player name or platform id, 1-64 characters"})
		}
	case "broadcast":
		if req.Message == "" || len(req.Message) > maxAgentMessageLen {
			fields = append(fields, FieldError{Field: "message", Message: "1-200 characters"})
		}
		if req.Style != "" && req.Style != "center" && req.Style != "topleft" {
			fields = append(fields, FieldError{Field: "style", Message: "center or topleft"})
		}
	case "say":
		if req.Message == "" || len(req.Message) > maxAgentMessageLen {
			fields = append(fields, FieldError{Field: "message", Message: "1-200 characters"})
		...
		if len(req.Name) > 32 {
			fields = append(fields, FieldError{Field: "name", Message: "at most 32 characters"})
		}
	case "time":
		set := 0
		if req.Skip != "" {
			set++
			if req.Skip != "morning" {
				fields = append(fields, FieldError{Field: "skip", Message: "the only supported value is morning"})
			}
		}
		if req.Fraction != nil {
			set++
			if *req.Fraction < 0 || *req.Fraction > 1 {
				fields = append(fields, FieldError{Field: "fraction", Message: "between 0 and 1"})
			}
		}
		if req.Seconds != nil {
			set++
			if *req.Seconds < 1 || *req.Seconds > 86400 {
				fields = append(fields, FieldError{Field: "seconds", Message: "between 1 and 86400"})
			}
		}
		if set != 1 {
			fields = append(fields, FieldError{Field: "time", Message: "set exactly one of fraction, skip or seconds"})
		}
	case "setkey", "removekey":
		if !agentKeyPattern.MatchString(req.Key) {
			fields = append(fields, FieldError{Field: "key", Message: "must match [A-Za-z0-9_]{1,64}"})
		}
	case "event":
		if req.Event == "" || len(req.Event) > 64 {
			fields = append(fields, FieldError{Field: "event", Message: "event name, 1-64 characters"})
		}
		if (req.X == nil) != (req.Z == nil) {
			fields = append(fields, FieldError{Field: "x", Message: "set both x and z, or neither"})
		}
	}
	return fields
}
